...
7. 发布您的版本
    ...
    ```
```
git-cliff --config git-cliff.toml --output resource/CHANGELOG.md #生成更新变动文件
不写标签，手动运行yml可以生成非正式版
git tag v2.0.2 -f -m "强制覆盖标签"  # 强制创建并覆盖已有标签
git push origin v2.0.2 --force        # 强制推送远程标签
```
